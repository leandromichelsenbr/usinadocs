...
final class TableArtifactTest extends TestCase {
 ...
 public function test_table_block_is_preserved_and_rendered_when_published():void{$db=Database::connect($this->path);$service=new EditorialService($db);$page=$service->create('Tabela','tabela','','','');$service->replaceDraftBlocks($page,'pt',[['type'=>'table','data'=>['headers'=>['Função','Uso'],'rows'=>[['Len','Tamanho'],['Empty','Validação']]]]]);$service->publish($page);$blocks=(new PublishedPageRepository($db))->findByLocalizedSlug('pt','tabela')['blocks'];self::assertSame('table',$blocks[0]['type']);self::assertSame('Função',$blocks[0]['data']['headers'][0]);self::assertSame('Validação',$blocks[0]['data']['rows'][1][1]);$body=AppFactory::create(dirname(__DIR__),$this->path)->handle((new ServerRequestFactory())->createServerRequest('GET','/pt/p/tabela'))->getBody()->__toString();self::assertStringContainsString('<th scope="col">Função</th>',$body);self::assertStringContainsString('th{background:#1c3854;color:#f2f7ff}',$body);}
}
