...
final class QuizArtifactTest extends TestCase {
 ...
 public function test_quiz_renders_options_and_feedback_data():void{$db=Database::connect($this->path);$service=new EditorialService($db);$page=$service->create('Quiz','quiz','','','');$service->replaceDraftBlocks($page,'pt',[['type'=>'quiz','data'=>['question'=>'Qual comando repete um bloco?','options'=>['IF','FOR','RETURN'],'correct'=>1,'explanation'=>'FOR...NEXT executa um bloco repetidamente.','error_explanation'=>'Reveja como laços de repetição funcionam.','error_url'=>'/pt/p/for-next']]]);$service->publish($page);$body=AppFactory::create(dirname(__DIR__),$this->path)->handle((new ServerRequestFactory())->createServerRequest('GET','/pt/p/quiz'))->getBody()->__toString();self::assertStringContainsString('class="quiz" data-answer="1"',$body);self::assertStringContainsString('FOR...NEXT executa um bloco repetidamente.',$body);self::assertStringContainsString('Reveja como laços de repetição funcionam.',$body);self::assertStringContainsString('href="/pt/p/for-next"',$body);self::assertStringContainsString('Resposta correta.',$body);}
}
