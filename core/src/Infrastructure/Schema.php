...
final class Schema
{
    ...
    public static function seed(PDO $database): void
    {
        ...
        $models = [
            ['model-reference', 'reference', 'Referência técnica', 'Sintaxe, explicação, exemplos e fontes para consulta rápida.', [['signature','Assinatura técnica',0],['heading','Título de seção',0],['text','Texto',1],['code','Código',0],['table','Tabela',0],['diagram','Diagrama',0],['image','Imagem',0],['reference','Referência',1],['citation','Citação',0],['notice','Aviso',0]]],
        ...
        $database->exec("UPDATE editorial_model_artifacts SET position=position+1 WHERE model_id='model-reference' AND NOT EXISTS (SELECT 1 FROM editorial_model_artifacts WHERE model_id='model-reference' AND artifact_type='signature')");
        ...
        $database->exec("UPDATE editorial_model_revision_artifacts SET position=position+1 WHERE model_revision_id='model-revision-reference-1' AND NOT EXISTS (SELECT 1 FROM editorial_model_revision_artifacts WHERE model_revision_id='model-revision-reference-1' AND artifact_type='signature')");
        foreach ($models as [$id, $type, $label, $description, $artifacts]) {
