...
final class ReusableArtifactService
{
    ...
    {
        return $this->db->query("SELECT a.id,a.slug,a.artifact_type,COALESCE(d.title,p.title) AS title,CASE WHEN d.id IS NOT NULL THEN 'draft' ELSE 'published' END AS status,CASE WHEN endraft.id IS NOT NULL THEN 'draft' WHEN en.published_revision_id IS NULL THEN 'absent' WHEN enpublished.source_revision_id<>a.published_revision_id THEN 'outdated' ELSE 'published' END AS en_status,CASE WHEN esdraft.id IS NOT NULL THEN 'draft' WHEN es.published_revision_id IS NULL THEN 'absent' WHEN espublished.source_revision_id<>a.published_revision_id THEN 'outdated' ELSE 'published' END AS es_status FROM reusable_artifacts a LEFT JOIN reusable_artifact_revisions d ON d.artifact_id=a.id AND d.status='draft' LEFT JOIN reusable_artifact_revisions p ON p.id=a.published_revision_id LEFT JOIN reusable_artifact_localizations en ON en.artifact_id=a.id AND en.language_code='en' LEFT JOIN reusable_artifact_translation_revisions enpublished ON enpublished.id=en.published_revision_id LEFT JOIN reusable_artifact_translation_revisions endraft ON endraft.artifact_id=a.id AND endraft.language_code='en' AND endraft.status='draft' LEFT JOIN reusable_artifact_localizations es ON es.artifact_id=a.id AND es.language_code='es' LEFT JOIN reusable_artifact_translation_revisions espublished ON espublished.id=es.published_revision_id LEFT JOIN reusable_artifact_translation_revisions esdraft ON esdraft.artifact_id=a.id AND esdraft.language_code='es' AND esdraft.status='draft' ORDER BY COALESCE(d.title,p.title)")->fetchAll(PDO::FETCH_ASSOC);
    }

    public function translationReview(string $search=''):array{return array_values(array_filter($this->catalog(),static fn(array $artifact):bool=>$search===''||stripos($artifact['title'],$search)!==false));}
    ...
    public function refreshTranslation(string $artifactId,string $language):?string{if(!in_array($language,['en','es'],true))return null;$existing=$this->translationDraft($artifactId,$language);if($existing!==null)return$existing['revision_id'];$statement=$this->db->prepare('SELECT r.*,a.published_revision_id AS current_source FROM reusable_artifact_localizations l JOIN reusable_artifact_translation_revisions r ON r.id=l.published_revision_id JOIN reusable_artifacts a ON a.id=l.artifact_id WHERE l.artifact_id=:id AND l.language_code=:language');$statement->execute(['id'=>$artifactId,'language'=>$language]);$published=$statement->fetch(PDO::FETCH_ASSOC);if($published===false||$published['current_source']===null)return null;$revision='artifact-translation-revision-'.bin2hex(random_bytes(8));$this->db->prepare("INSERT INTO reusable_artifact_translation_revisions (id,artifact_id,language_code,number,status,source_revision_id,title,description,data,created_at) VALUES (:revision,:id,:language,:number,'draft',:source,:title,:description,:data,:created)")->execute(['revision'=>$revision,'id'=>$artifactId,'language'=>$language,'number'=>(int)$published['number']+1,'source'=>$published['current_source'],'title'=>$published['title'],'description'=>$published['description'],'data'=>$published['data'],'created'=>gmdate('c')]);return$revision;}
