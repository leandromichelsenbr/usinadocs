...
final class AppFactory
{
    public static function create(string $root, ?string $databasePath = null): \Slim\App
    {
        ...
        $app->get('/admin/translations', static function (ServerRequestInterface $request, ResponseInterface $response) use ($twig, $editorial): ResponseInterface {if(!isset($_SESSION['user'])||$_SESSION['user']['role']!=='administrator')return$response->withHeader('Location','/login')->withStatus(302);$search=(string)($request->getQueryParams()['q']??'');return$twig->render($response,'translation-review.twig',['pages'=>$editorial->translationReview($search),'search'=>$search]);});
